  ...
  LateralHorizon & ctrl_cmd_horizon)
{
  ...
  // ctrl_cmd.steering_tire_angle = -0.22;  // Sujeendra changed
  ...
  for (auto it = std::next(Uex.begin()); it != Uex.end(); ++it) {
    ...
    // RCLCPP_ERROR(m_logger, "Steering tire angle: %f", lateral.steering_tire_angle);
...
{
  // Trajectory  = msg;
  // // Sujeendra Change implement trajectory change
  // for (auto & point : trajectory_msg.points) {
  //   point.pose.position.y -= 8;
  // }
