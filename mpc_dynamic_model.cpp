  ...
  const double dt)
{
   ...
   */
  // RCLCPP_ERROR(
  //   rclcpp::get_logger("control.trajectory_follower.lateral_controller"), "I am here %f", dt);
  const double vel = std::max(m_velocity, 0.01);

  a_d = Eigen::MatrixXd::Zero(m_dim_x, m_dim_x);
  a_d(0, 1) = 1.0;
  a_d(1, 1) = -2 * (m_cf + m_cr) / (m_mass * vel);
  a_d(1, 2) = 2 * (m_cf + m_cr) / m_mass;
  a_d(1, 3) = 2 * (m_lr * m_cr - m_lf * m_cf) / (m_mass * vel);
  a_d(2, 3) = 1.0;
  a_d(3, 1) = 2 * (m_lr * m_cr - m_lf * m_cf) / (m_iz * vel);
  a_d(3, 2) = 2 * (m_lf * m_cf - m_lr * m_cr) / m_iz;
  a_d(3, 3) = -2 * (m_lf * m_lf * m_cf + m_lr * m_lr * m_cr) / (m_iz * vel);
  ...
  b_d(0, 0) = 0.0;
  b_d(1, 0) = 2 * m_cf / m_mass;
  b_d(2, 0) = 0.0;
  b_d(3, 0) = 2 * m_lf * m_cf / m_iz;

  w_d = Eigen::MatrixXd::Zero(m_dim_x, 1);
  w_d(0, 0) = 0.0;
  w_d(1, 0) = 2 * (m_lr * m_cr - m_lf * m_cf) / (m_mass * vel) - vel;
  w_d(2, 0) = 0.0;
  w_d(3, 0) = -2 * (m_lf * m_lf * m_cf + m_lr * m_lr * m_cr) / (m_iz * vel);
